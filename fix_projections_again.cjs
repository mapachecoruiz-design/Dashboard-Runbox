...
// Identify the start of tbody and the start of </table>
const startIndex = code.indexOf('<tbody className="divide-y divide-slate-100">');
const endIndex = code.indexOf('            </table>');
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  const finalCode = code.slice(0, startIndex) + newTbody + code.slice(endIndex);
